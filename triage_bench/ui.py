...
ALL_MODELS = OLD_CHOICES + GROQ_CHOICES
...
def run_benchmark_ui(model, runs, num_vignettes, key_openai, key_deepseek, key_groq, progress=gr.Progress()):
    ...
    """
    if num_vignettes is not None and num_vignettes <= 0:
        return "Error: Number of vignettes must be positive or empty for all vignettes.", "Error"
    
    # Convert empty string to None for num_vignettes
    if num_vignettes == "" or num_vignettes == 0:
        num_vignettes = None
    ...
    try:
        ...
        def progress_callback(update_dict):
            ...
            if progress is not None:
                progress(completed / total, desc=f"Run {run}/{runs_total}, Case {current_case}/{total_cases}, Total: {completed}/{total}")
...
def create_ui():
    ...
    with gr.Blocks(title="Triage Benchmark") as demo:
        ...
        with gr.Row():
            with gr.Column():
                ...
                num_vignettes_input = gr.Number(
                    ...
                    info="Leave empty or 0 to use all vignettes",
                    precision=0
                )
                
                with gr.Accordion("API Keys (optional - leave empty if already set in environment)", open=False):
                    key_openai_input = gr.Textbox(
                        label="OpenAI API Key",
                        type="password",
                        placeholder="sk-...",
                        info="Required for OpenAI models (gpt-4o, o1, o3, etc.)"
                    )
        ...
        def run_with_progress(model, runs, num_vignettes, key_openai, key_deepseek, key_groq, progress=gr.Progress()):
            """Wrapper to run benchmark with progress tracking."""
            result, status = run_benchmark_ui(
                model, runs, num_vignettes, 
                key_openai, key_deepseek, key_groq,
                progress
            )
        ...
        run_button.click(
            ...
            inputs=[
                ...
                key_openai_input, key_deepseek_input, key_groq_input
            ],
